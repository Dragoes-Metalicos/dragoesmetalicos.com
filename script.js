...
// Geographical Location & Experience Detection System
let currentLang = 'pt';
...
async function initGeographicalDetection() {
    const geoTextElement = document.getElementById('geoText');
    if (!geoTextElement) return;

    // Check sessionStorage for cached detection
    const cachedLang = sessionStorage.getItem('detectedLang');
    const cachedLabel = sessionStorage.getItem('detectedLabel');

    if (cachedLang && cachedLabel) {
        currentLang = cachedLang;
        geoTextElement.textContent = cachedLabel;
        updateLanguage();
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data && data.country_code) {
                ...
                const label = city ? `${city}, ${countryCode}` : countryCode;
                ...
                sessionStorage.setItem('detectedLang', currentLang);
                sessionStorage.setItem('detectedLabel', label);
                geoTextElement.textContent = label;
                updateLanguage();
                return;
            }
        }
    } catch (error) {
        console.warn('Geographical detection failed. Falling back to browser settings.', error);
    }

    // Fallback: Browser language
    const browserLang = navigator.language || navigator.userLanguage;
    let fallbackLang = 'en';
    let fallbackLabel = 'Global';
    ...
    geoTextElement.textContent = fallbackLabel;
    sessionStorage.setItem('detectedLang', currentLang);
    sessionStorage.setItem('detectedLabel', fallbackLabel);
    updateLanguage();
}
...
async function loadDonors() {
    ...
    // Avoid double loading if the language didn't change
    if (lastLoadedLang === currentLang) return;
